...
public class newsDetails extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        NewsDAO x = new NewsDAO();
        String newsId = request.getParameter("id");

        //listnews details
        HttpSession session = request.getSession();
        List<News> lst = x.getListNews();
        session.setAttribute("lstNewsDt", lst);

        // Kiểm tra xem newsId có null không
        if (newsId != null) {
            News a = x.findNewsById(newsId);
            request.setAttribute("details", a);
        }

        String url = "client/NewsDetails.jsp";
