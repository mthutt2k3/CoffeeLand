...
public class SortProductList extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SortProductListClient</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SortProductListClient at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        String type = request.getParameter("productlist");
        if(type.equalsIgnoreCase("newest")){
            plst = pDAO.getSortListProduct("productId", "desc");
        }else if(type.equalsIgnoreCase("most")){
            plst = pDAO.getSortListProduct("count", "desc");
        }else if(type.equalsIgnoreCase("lowtohigh")){
            plst = pDAO.getSortListProduct("price", "asc");
        }else if(type.equalsIgnoreCase("hightolow")){
            plst = pDAO.getSortListProduct("price", "desc");
        }else{
            plst = pDAO.getListProduct();
        }
        List<Category> clst = cDAO.getListCategory();
        
        session.setAttribute("productlist", type);
