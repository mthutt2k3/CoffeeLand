...
public class CreateCustomer extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Users x = new Users(userName, name, phone, email, password, address, "xxx", "4", "1");
        if(!password.equals(cPassword)){
            String alert = "Mật khẩu nhập lại không đúng";
            session.setAttribute("x", x);
            session.setAttribute("alert", alert);
            String url = "/CoffeeLand/server/Users/CreateCustomer.jsp";
            ...
            String url = "/CoffeeLand/server/Users/CreateCustomer.jsp";
            response.sendRedirect(url);
        } else if (c.isExistEmail(email, "4")) {
            String alert = "Email đã tồn tại";
            session.setAttribute("x", x);
            session.setAttribute("alert", alert);
            String url = "/CoffeeLand/server/Users/CreateCustomer.jsp";
            response.sendRedirect(url);
        } else if (c.isExistPhone(phone, "4")) {
            ...
            c.insert(x);
            String alert = "Tạo khách hàng mới thành công!";
            session.setAttribute("alert", alert);
            String url = "usermanageforsaler";
            response.sendRedirect(url);
        }
