<%-- 
    Document   : AddNewFeedback
    Created on : Mar 12, 2024, 8:04:38 AM
    Author     : dell
--%>
...
<html>
    ...
    <style>
        ...
        table td, table th {
            padding: 8px;
            text-align: left;
            border-bottom: 1px solid #ddd; /* tạo viền dưới cho mỗi hàng */
        }
        ...
        .feddel {
            text-align: center; /* căn giữa nút "Xoá đánh giá" */
        }
        ...
        .deleteform input[type="submit"] {
            background-color: #f44336; /* màu nền cho nút "Xoá đánh giá" */
            color: white; /* màu chữ */
...
.dropdown:hover .dropdown-menu {
    ...
    <body>
        ...
        <div class="container-fb">
            ...
            %>
            <h1>Thêm đánh giá của bạn tại đây:</h1>

            <form action="/CoffeeLand/addfeedback" method="post" enctype="multipart/form-data" onsubmit="return validateRating()">
                <input type="hidden" name="userId" value="<%= userId %>"/>
                <input type="hidden" name="productId" value="<%= productId %>"/>
                <div>
                    <label for="feedbackContent">Nội dung phản hồi:</label><br>
                    <textarea id="feedbackContent" name="feedbackContent" rows="4" cols="50" required></textarea>
                </div>

                <!-- Upload hình ảnh -->
                <div>
                    <label for="imageUpload">Chọn hình ảnh (nếu có):</label><br>
        ...
        List<Feedback> listF = fbd.getAllFeedbackByUIDPID(userId1, productId1);
        if(listF.isEmpty()){
            %><p class="title">Chưa có đánh giá trước đó</p><%
                }else{
            %>
            <h1>Đánh giá trước đó của bạn</h1>
            <table>
                <tbody>
                    <%
                    ...
                    <tr>
                        ...
                        <td>
                            <p><%= fb.getFeedbackTime() %></p>
                        </td>
                        <td>
                            <% if (fb.getImage() != null && !fb.getImage().isEmpty()) { %>
                            <img style="width:100px; height: 80px;margin-left:50px;border-radius: 10px" src="/CoffeeLand/assests/image/feedback/<%= fb.getImage() %>" alt="alt"/>
                            <% } else { %>
                            &nbsp;
                            <% } %>
                        </td>
                        <td>
                            ...
                            <p><a href="/CoffeeLand/client/feedback/EditFeedback.jsp?userId=<%= userId %>&productId=<%= productId %>&postedTime=<%= fb.getFeedbackTime() %>">Sửa đánh giá</a></p>
