<%-- 
    Document   : NewsManage
    Created on : Feb 13, 2024, 10:21:18 AM
    Author     : acer
--%>
                ...
                <div class="row bg-bodylist" style="height: fit-content">
                    ...
                    <div class="mb-3 mt-2">
                        <form class="search-form mx-auto" action="/CoffeeLand/control" method="GET">
                            <input type="hidden" name="target" value="NewsManage"/>
                            <input type="hidden" name="requestAction" value="Search"/>
                            <input type="search" name="input" class="search-inputlist form-control" placeholder="Tìm kiếm tin tức...">
                            <button type="submit" class="search-button"></button>
                        </form>
                    </div>

                    <form action="/CoffeeLand/server/news/AddNews.jsp">
                        <button type="submit" 
                    ...
                    <div class="col-md-12">
                        <table class="table table-hover table-bordered">
                            ...
                            <tr class="table-infolist">
                                ...
                                <th class="col-md-2 text-center">
                                    <div style="display: flex">
                                        Tiêu đề
                                        <%colName = "title";%>
                                        <%@include file="/server/news/NewsSortForm.jsp"%>
                                    </div>
                                </th>
                                <th class="col-md-2 text-center">
                                    <div style="display: flex">
                               ...
                               aria-labelledby="typeDropdown">
                                <li>
                                    <form action="/CoffeeLand/control">
                                        <input type="hidden" name="requestAction" value="Get News Priority Type">
                                        <input type="hidden" name="target"value="NewsManage">
                            ...
                            <c:choose>
                                <c:when test="${empty lstNews}">
                                    <div class="row h-auto w-auto mt-2" style="padding-left: 100px; padding-right: 100px">
                                        <h1 class="text-warning text-center">Không có tin tuc</h1>
                                    </div>
                                </c:when> 
                                <c:otherwise>

                                    <c:forEach items="${lstNews}" var="x">
                                        <tr>
                                            <td class="text-light text-center">${x.getNewsId()}</td>
                                            <td class="text-light text-center"><b>${x.getTitle()}</b></td>
                                            <td class="text-light text-center">${x.getPostTime()}</td>
                                            <td class="text-light text-center">${x.getUserName()}</td>
                                            
                                            <td class="text-light text-center">
                                                <c:choose>
                                                    <c:when test="${'High'.equalsIgnoreCase(x.getPriorityName())}">
                                                        Cao
                                                    </c:when>
                                                    <c:when test="${'Medium'.equalsIgnoreCase(x.getPriorityName())}">
                                                        Trung bình
                                                    </c:when>
                                                    <c:when test="${'Low'.equalsIgnoreCase(x.getPriorityName())}">
                                                        Thấp
                                                    </c:when>
                                                </c:choose>
                                            </td>
                                            ...
                                            <td class="text-light text-center" style="display: flex; justify-content: center; align-items: center;">
                                                <div class="col-md-6 side-image" style="height: 50px; width: 70px;">
                                                    <img style="height: 100%; width: 100%; object-fit: contain;" src='/CoffeeLand/assests/image/thumbnails/${x.getImage()}'>
                                                    <br/>
                                                </div>
                                            </td>
                                            <td>
                                                <form action="/CoffeeLand/updatenews" method="post">
                                                    <input type="hidden" name="newsId" value="${x.getNewsId()}">
                                                    <input type="hidden" name='requestAction' value="Update News"/>
                        ...
                        <ul class="pagination">
                            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                <a class="page-link" href="/CoffeeLand/newslist?page=1">Trang 1</a>
                            </li>
                            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                <a class="page-link" href="/CoffeeLand/newslist?page=${currentPage - 1}">Trang trước</a>
                            </li>
                            <c:forEach begin="1" end="${totalPages}" var="page">
                                <li class="page-item ${page == currentPage ? 'active' : ''}">
                                    <a class="page-link" href="/CoffeeLand/newslist?${page}">${page}</a>
                                </li>
                            </c:forEach>
                            <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                <a class="page-link" href="/CoffeeLand/newslist?page=${currentPage + 1}">Trang sau</a>
                            </li>
                            <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                <a class="page-link" href="/CoffeeLand/newslist?page=${totalPages}">Trang cuối</a>
                            </li>
                        </ul>
                    </div>
                    </div>
